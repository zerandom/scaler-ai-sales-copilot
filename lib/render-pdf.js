...
export async function renderPdf(assetData, strategy, leadProfile) {
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    const pg = pdfDoc.addPage([W, H]);
    r(pg, 0, 0, W, H, white);
    slideHeader(pg, "01", assetData.slide1Title || "Where You Stand Today", assetData.slide1Subtitle || "A snapshot of your current situation");

    const items = (assetData.situationItems || []).slice(0, 5);
    const cardH = 78;
    const cardW = W - M * 2;
    const totalH = items.length * cardH + (items.length - 1) * 8;
    let iy = H - 90;
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    const pg = pdfDoc.addPage([W, H]);
    r(pg, 0, 0, W, H, white);
    slideHeader(pg, "02", assetData.slide2Title || "Your Goals", assetData.slide2Subtitle || "What you want to achieve next");
    ...
    if (assetData.pullQuote) {
      r(pg, M, 44, W - M * 2, 44, chipBg);
      r(pg, M, 44, 4, 44, blue);
      let qy2 = 74;
      for (const l of wrapText(`"${assetData.pullQuote}"`, bold, 10, W - M * 2 - 30).slice(0, 2)) {
        safe(pg, l, { x: M + 16, y: qy2, size: 10, font: bold, color: blue });
        qy2 -= 16;
      }
    }
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    const pg = pdfDoc.addPage([W, H]);
    r(pg, 0, 0, W, H, white);
    slideHeader(pg, "03", assetData.slide3Title || "Your Key Questions, Answered", assetData.slide3Subtitle || "Honest answers to what matters most to you");
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    const pg = pdfDoc.addPage([W, H]);
    r(pg, 0, 0, W, H, white);
    slideHeader(pg, "04", assetData.slide4Title || "Why Scaler is the Right Fit for You", assetData.slide4Subtitle || "Designed for engineers. Built for outcomes.");
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    ...
    r(pg, cx2, cy2, 4, ch2, blue); // left accent bar
    // Title (no overlapping icon badge)
    safe(pg, assetData.nextStepTitle || "Take the Assessment", { x: cx2 + 20, y: cy2 + ch2 - 26, size: 15, font: bold, color: navy });
    ...
    for (const p of ["Personalized feedback", "Right batch recommendation", "Clarity on your readiness"]) {
      ...
      safe(pg, "v " + p, { x: px2 + 5, y: pr + 3, size: 7.5, font: bold, color: blue });
      px2 += pw + 6;
    }
    ...
    for (const item of (assetData.whyTakeIt || []).slice(0, 4)) {
      ...
      safe(pg, "v", { x: M + 7, y: ly + 2, size: 7, font: bold, color: goodGreen });
      safe(pg, item, { x: M + 26, y: ly, size: 9.5, font, color: darkGray });
      ly -= 22;
    }

    // Staircase illustration (right side, more prominent)
    const stX = leftW + 24;
    const stBase = 82;
    const stepW = 80, stepH = 58;
    for (let i = 0; i < 5; i++) {
      r(pg, stX + i * stepW * 0.76, stBase + i * stepH * 0.7, stepW, stepH, chipBg);
    }
    // Climber: head (circle) + body line + arms line
    const clX = stX + 4 * stepW * 0.76 + stepW / 2 + 10;
    const clY = stBase + 4 * stepH * 0.7 + stepH + 26;
    circ(pg, clX, clY, 10, orange);             // head
    pg.drawLine({ start: { x: clX, y: clY - 10 }, end: { x: clX, y: clY - 36 }, thickness: 3, color: orange }); // body
    pg.drawLine({ start: { x: clX - 14, y: clY - 20 }, end: { x: clX + 14, y: clY - 20 }, thickness: 3, color: orange }); // arms
    pg.drawLine({ start: { x: clX, y: clY - 36 }, end: { x: clX - 10, y: clY - 55 }, thickness: 3, color: orange }); // left leg
    pg.drawLine({ start: { x: clX, y: clY - 36 }, end: { x: clX + 10, y: clY - 55 }, thickness: 3, color: orange }); // right leg
